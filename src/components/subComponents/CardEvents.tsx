  ...
  return (
    <div className={style.container}>
      {listingEvents.map((event:IEvent) => (
        <div key={event.name} className={style.container_card} onClick={()=> handleEventClick(event)}>
          <div className={style.container_one}>
            <section>
              <p>{obtenerMes(event.date).nombreMes}</p>
              <p className={style.date}>{event.date.substring(8, 10)}</p>
            </section>
            <img src="https://img.freepik.com/premium-photo/backgrounds-night-sky-with-stars-clouds_180633-4011.jpg" alt={event.description} className={style.img_card} />
          </div>
